...
const areaInspectionModel = require('../models/area_inspection_model');
const { isDigit } = require('../utils/isdigit');

module.exports = {
  async index(req, res) {

    const areas = await areaInspectionModel.findAll();
  ...
  async deletebyidinspecao(req, res) {
    const {id} = req.params;
    const response =  await areaInspectionModel.deletebyidinspecao(id);
    res.json(response);
  }
}
